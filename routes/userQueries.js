...
router.post("/insertpost", (req, res) => {
  ...
  try {
    connection.query(
      `INSERT INTO post (artistId, title, desc2, imageUrl) VALUES (${artistId},'${title}','${desc}', '${imageUrl}')`,
      (error, res2) => {
        if (error) throw error;
...
router.post("/insertclass", (req, res) => {
  ...
  try {
    connection.query(
      `INSERT INTO class (hostId, title, desc2, imageUrl, date, duration, numOfSpots, location, eventType) VALUES (${hostId},'${title}','${desc}', '${imageUrl}', '${date}', ${duration}, ${numOfSpots}, '${location}', ${eventType})`,
      (error, res2) => {
        if (error) throw error;
...
router.get("/fetchspecificsessionsbyid/:id", async (req, res) => {
  ...
  try {
    connection.query(
      `SELECT session.id as sessionId, session.artistId, session.artistUid, session.studentId, session.studentUid, user.name as artistName,session.dateAndTime, session.isBooked, session.isRequested, session.isFinished,
      session.comment
